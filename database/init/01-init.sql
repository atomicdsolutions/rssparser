-- RSS Feed Parser Database Schema for Docker PostgreSQL
-- This is a simplified version for local PostgreSQL (without Supabase-specific features)
...
CREATE INDEX IF NOT EXISTS idx_feed_items_link ON feed_items(link);
...
CREATE INDEX IF NOT EXISTS idx_feed_logs_feed_id ON feed_processing_logs(feed_id);
CREATE INDEX IF NOT EXISTS idx_feed_logs_status ON feed_processing_logs(status);
CREATE INDEX IF NOT EXISTS idx_feed_logs_created_at ON feed_processing_logs(created_at DESC);
...
-- Feed summary view
CREATE OR REPLACE VIEW feed_summary AS
SELECT 
    f.id,
    f.url,
    f.name,
...
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO feedparser;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO feedparser;